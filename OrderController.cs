        ...
        public async Task<IActionResult> DeleteOrder(int orderId)
        {
            using (var connection = new SqliteConnection(_connectionString))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    try
                    {
                        const string deleteHistoryQuery = "DELETE FROM History_tb WHERE OrderId = @OrderId";
                        const string deleteOrderQuery = "DELETE FROM Order_tb WHERE OrderId = @OrderId";

                        // Delete from History_tb first to avoid foreign key constraint issues
                        await connection.ExecuteAsync(deleteHistoryQuery, new { OrderId = orderId }, transaction);
                        await connection.ExecuteAsync(deleteOrderQuery, new { OrderId = orderId }, transaction);
